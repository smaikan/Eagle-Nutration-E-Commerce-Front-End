...
const Item = ({item}) => {
  return (
    <div className='flex justify-center flex-wrap'>
        {item.map((item)=>(
         <Link key={item.productId} to={`/product/${item.productId}`} className='w-[230px] h-[340px] pt-4 mb-12 flex flex-col mx-8 items-center  rounded-xl  hover:shadow-2xl transition duration-300'  key={index}>
