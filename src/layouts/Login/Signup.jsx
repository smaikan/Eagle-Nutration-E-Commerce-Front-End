...
const Signup = ({setlogin}) => {
  ...
  if (
    formData.password !== formData.againpsw ||
    !formData.password ||
    !formData.againpsw ||
    !formData.email ||
    !formData.name
  ) {
    toast.error("Lütfen tüm alanları doldurun ve şifreleri kontrol edin!", {
    ...
    <div className="w-[550px] transition-all border-0 border-gray-600 rounded-lg flex justify-center ">
      <div className="px-5 w-[500px] flex flex-col ">
        ...
        <buttons className='flex justify-between'>
          ...
          <div onClick={()=>SaveUser()} className="cursor-pointer mb-10 mt-1 font-extrabold text-2xl rounded-full flex justify-center items-center text-white  w-48 h-14 bg-gray-600">
          Üye ol
        </div>
